...
Гра повинна має бути у образу стрічки (TAP) або снепшоту (SNA).

Якщо в наявності є лише TZX-файл, то його треба сконвертувати у TAP-файл (якщо лоадер використовує стандартний аудіоформат). Я зазвичай це роблю за допомогою емулятора [ZEsarUX](https://github.com/chernandezba/zesarux):

```
zesarux.exe --convert-tzx-tap "filein.tzx" "fileout.tap"
```

В інакшому випадку потрібно запустити гру в якому-небудь емуляторі ZX Spectrum та зберегти її у вигляді стандартного снепшоту (SNA).

Інколи гра не може завантажитись і з стандартного TAP-файлу (коли гра записана у стандартному аудіоформаті, але лоадер використовує компрессію даних). В цьому випадку варто спробувати звавантажити гру у вигляді SNA-снепшоту.

Також, якщо гра завантажилась і ніби працює, але не реагує ні на які клавіші (навіть не можна викликати меню емулятора по `F1`), то можна спробувати запустити гру у вигляді SNA-снепшоту.

Якщо гра все ж завантажилась і ми попали у меню, то це ще не кінець.

Треба запустити гру і трішки пограти. Найчастіше гра може зависнути при переході у іншу кімнату, інший рівень, або після Game Over. Також, інколи гра може зависнути просто через деякий час (5-10 хвилин). 

Проводити перевірку бажано в такому порядку:

1. SPEmu
2. SPEmu (ROM-версія)
3. SPEmu128
4. SPEmu128 (ROM-версія)

якщо є окремі версії гри для Спектруму 48K та 128K, то 48K-версію гри теж можна спробувати і в емуляторі моделі 128k. До того ж, якщо в меню гри є опція вимкнення музики, то можна спробувати почати гру без музики.

Тобто повний список виглядає так (SNA-використовувати, якщо гра не може навіть завантажитись):

1. TAP**48**k у **SPEmu**
2. TAP**48**k у **SPEmu** (ROM-версія)
3. SNA**48**k у **SPEmu**
4. SNA**48**k у **SPEmu** (ROM-версія)
5. TAP**128**k у **SPEmu128**
6. TAP**128**k у **SPEmu128** (ROM-версія)
7. SNA**128**k у **SPEmu128**
8. SNA**128**k у **SPEmu128** (ROM-версія)
9. TAP**48**k у **SPEmu128**
10. TAP**48**k у **SPEmu128** (ROM-версія)
11. SNA**48**k у **SPEmu128**
12. SNA**48**k у **SPEmu128** (ROM-версія)
