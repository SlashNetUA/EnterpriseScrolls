...
However, because you do not want to be bothered with unnecessary commands, the Enterprise uses 'default' channels whenever it can. Normally when you [PRINT](commands/man_cs-print.md), for example, you want the words or numbers to appear on the TV screen. So `PRINT "Hello"` puts the message on the display.
...
For more details on channels, refer to the chapter on commands in the Reference Section, particularly under the heading [OPEN](commands/man_cs-open.md).