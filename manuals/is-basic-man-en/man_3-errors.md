...
You can now move the cursor up and edit line 300. The functions [EXLINE](functions/man_fn-exline.md), [EXTYPE](functions/man_fn-extype.md), and [EXSTRING$](functions/man_fn-exstring.md) are supplied to help in the handling of errors and other exceptions. Each error has its own number, which can be referenced with [EXTYPE](functions/man_fn-extype.md), and for most of these errors a special message will be printed if it is not suppressed.
...
Error messages can be trapped, if desired, by using [WHEN EXCEPTION](commands/man_cs-when.md) and a handler block (page 134). An exception handler can be used to trap any error, even those such as a memory overflow or a syntax error (a keyword mis-spelled, for example). This must be handled with care, as a [RETRY](commands/man_cs-retry.md) to a permanent error will cause the program to loop indefinitely.

Errors like a division by zero, or a negative [SQR](functions/man_fn-sqr.md) argument, can be caught without crashing the program.
