...
Two ways of branching within a program are [GOTO](commands/man_cs-goto.md) and [GOSUB](commands/man_cs-gosub.md). The other main method of branching provided on this computer is that of functions, which are dealt with on page 79.
[GOTO](commands/man_cs-goto.md) and [GOSUB](commands/man_cs-gosub.md) both make the computer jump to another part of the program from the part it has reached when it comes to either of these words.
...
[GOTO](commands/man_cs-goto.md) is a far more awkward way to split large programs up into separate components than the use of functions, described on page 79.

The clumsiness of [GOTO](commands/man_cs-goto.md) is mostly because it breaks up the normal order of program flow without providing an alternative structure. This is why programs get lost!

## GOSUB

[GOSUB](commands/man_cs-gosub.md) is similar to [GOTO](commands/man_cs-goto.md) in that it directs the computer to a line number in a different bit of the program, but with [GOSUB](commands/man_cs-gosub.md) you expect to return.

Using [GOSUB](commands/man_cs-gosub.md) you need to set part of a program aside as a 'unit' – a bit which is meant to do one thing only. Then, at the end of that bit of program, you must add the word [RETURN](commands/man_cs-return.md), to tell the computer to go back to the line immediately following the one that contains the word [GOSUB](commands/man_cs-gosub.md).

The bit of the program between the line indicated by a [GOSUB](commands/man_cs-gosub.md) command and the [RETURN](commands/man_cs-return.md) command is called a subroutine. A function can also be a subroutine, using the word [CALL](commands/man_cs-call.md).

## HOW TO AVOID MISTAKES

If you don't want to go wrong and become confused, please remember two points whenever you use [GOSUB](commands/man_cs-gosub.md)/[RETURN](commands/man_cs-return.md):

First, a [GOSUB](commands/man_cs-gosub.md) must never be used without a [RETURN](commands/man_cs-return.md). If this happens, the result is a message from the computer to tell you you've made a mistake.
...
If this happens, and the computer reads the word [RETURN](commands/man_cs-return.md) without having been told to [GOSUB](commands/man_cs-gosub.md) first, it will stop the program because it will not understand.
...
1080 RETURN
```
So the program above shows you how to use [GOSUB](commands/man_cs-gosub.md). But as you can see, it is far less tidy than a program using a function with [CALL](commands/man_cs-call.md) – notice for instance that apart from the [GOSUB](commands/man_cs-gosub.md) line there is no indication where the subroutine begins.

Programs which use [GOTO](commands/man_cs-goto.md) or [GOSUB](commands/man_cs-gosub.md) are more likely to go wrong than programs which use structured loops and functions.
