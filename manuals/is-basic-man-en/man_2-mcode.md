...
But note that [ALLOCATE](commands/man_cs-allocate.md) destroys all stored variable values, so it's best to use it only at the beginning of a program.

## CODE AND HEX$

The machine-code routine is stored – with a given name – by using the [CODE](commands/man_cs-code.md) command:

`CODE name = routine in hex codes`

The name has the same format as a variable name.

[CODE](commands/man_cs-code.md) can only store a string – your hex codes must be converted into the right format by using [HEX$](functions/man_fn-hex.md), as follows:
...
Note that the argument of [USR](functions/man_fn-usr.md) is passed into **HL** at the beginning of the routine; and the value returned by [USR](functions/man_fn-usr.md) is the contents of the **HL** register at the end of the routine.
