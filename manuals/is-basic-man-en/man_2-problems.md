...
3. an [END](commands/man_cs-end.md) statement (which marks the point where the computer will stop running the program; it need not be the final line in numerical order);
4. functions which handle parts of the main task;
5. [DATA](commands/man_cs-data.md) statements if used (these can also come towards the beginning).
...
[DATA](commands/man_cs-data.md) statements don't necessarily need to come at the end of a program, but they all ought to be in the same, fairly obvious, place. It can be fiddly to search through a lot of them or count how many there are. Putting them all together saves lime spent searching for them if one is wrong.

[DATA](commands/man_cs-data.md) statements can also be used within a function, as long as they are local to that function.

The [END](commands/man_cs-end.md) statement, of course, marks the end of the main controller program. As mentioned earlier in this chapter, it is not always the last line of a program. Where functions or subroutines are used, the computer 'jumps out of' the main program to use them, rather than working through in sequential order. The END statement should therefore be in the place where the controller program would actually finish.
...
You can also print words tidily on the screen by using [PRINT AT](commands/man_cs-print.md) to give margins, and [UCASE$](functions/man_fn-ucase.md) or [LCASE$](functions/man_fn-lcase.md) to tidy up strings which are typed in by the person using the program. Sound effects and colours can be used as signals.
