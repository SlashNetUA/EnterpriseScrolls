...
A program line may be up to **250** characters long, with a [line-number](commands/man_cs-linenum.md) **1** to **9999**. It may include several statements, separated by colons; anything permitted after **THEN** in an **IF**/**THEN** statement may be incorporated on a multi-statement line.
...
A program can be referred to either by number, or by a name given on a [PROGRAM](commands/man_cs-program.md) line. See in particular the commands [CHAIN](commands/man_cs-chain.md), [EDIT](commands/man_cs-edit.md) and [PROGRAM](commands/man_cs-program.md).

At any particular time, one of the programs (by default, program **0**) is the 'current' one, on which commands such as [LIST](commands/man_cs-list.md) and [RENUMBER](commands/man_cs-renumber.md) will operate. The number of this program is shown on the 'status line' at the top of the screen.
...
Strings have a maximum length of **254** characters, if declared to this length (see [STRING](commands/man_cs-string.md)). Sub-strings may be referenced in the form **string-id(x:y)**, which specifies a string beginning with character number **x**, and ending with character number **y**. If either **x** or **у** are omitted, they default to the **start** or **end** of the string respectively.
