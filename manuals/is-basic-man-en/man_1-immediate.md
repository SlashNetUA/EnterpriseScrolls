...
Now you know about programming in BASIC and about immediate mode, let's explain more about keywords. [PRINT](commands/man_cs-print.md) is one of these, and has a special meaning in BASIC. Keywords are the instructions to which the manual has referred before. There are many of them, and each one tells the computer to do one (or possibly more than one) small thing. [PRINT](commands/man_cs-print.md), for example, tells the computer to put a display on the screen.
