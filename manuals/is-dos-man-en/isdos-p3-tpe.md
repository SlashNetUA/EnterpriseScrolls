...
The default EXOS channel will normality be an editor channel, although if printer echo is enabled it will be a channel to a special internal device which directs its output to the editor and to the printer. This is the channel which the IS-DOS CLI uses for its input and output. All the supplied transient programs ([DISKCOPY](../dos-commands/cmd-diskcopy.md), [BACKUP](../dos-commands/cmd-backup.md), [CHKDSK](../dos-commands/cmd-chkdsk.md), [UNDEL](../dos-commands/cmd-undel.md) and [XDIR](../dos-commands/cmd-xdir.md)) also use this channel.
