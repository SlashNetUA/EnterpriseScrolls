---
title: "IS-DOS Manual. Appendix 3: List of CP/M Programs Tested"
---
# Appendix 3: List of CP/M Programs Tested 

| Program         | Version | Company     |
| --------------- |:-------:| ----------- |
| Wordstar        |   3.0   | Micropro    |
| Wordstar        |  3.30   | Micropro    |
| DataStar        |  1.41   | Micropro    |
| ReportStar      |  1.02   | Micropro    |
| CalcStar        |  1.45   | Micropro    |
| SuperCalc       |         |             |
| SuperCalc II    |         |             |
| SuperWriter     |         |             |
| dBase II        |  2.31   | Ashton Tate |
| Multiplan       |  1.05   | Microsoft   |
| MBasic          |   5.2   | Microsoft   |
| Turbo-Pascal    |   2.0   | Borland     |
| Turbo-Pascal    |   3.0   | Borland     |
| Turbo Database  |         | Borland     |
| Z80 figForth    |   1.1   |             |
| NPS MICRO-Cobol |   2.1   |             |

All programs must be on the disk in CP/M 80 or CP/M 2.2 MS-DOS format.

This list will be expanded as additional programs are successfully tested.