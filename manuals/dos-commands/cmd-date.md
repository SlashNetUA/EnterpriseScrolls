...
The  format  of  the  date  is  flexible,  and  is specified by [EXOS variable 79 DT_FORM](../../../programming/system-info/exos-variables/exos_var79.md), as described below. This allows  the  date  to be input in the form  of  the  **UK** format (`DD-MM-YY`),  the **American** format (`MM-DD-YY`), the **International**  format  (`YY-MM-DD`) or indeed  any  other  format. When inputting, the separators may be `,`, `-`, `.`,  `/`, `:` or **Space**, with spaces either side of the separator.  Leading  zeros on the date numbers are  not significant.  Any  missing fields will default  to  the current value.
...
See also [TIME](cmd-time.md) and [IS-Basic DATE](../is-basic-man-en/commands/man_cs-date.md) command.
