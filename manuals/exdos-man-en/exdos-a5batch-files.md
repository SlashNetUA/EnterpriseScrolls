...
EXDOS will recognize a file with the filename **EXDOS.INI** and will run this on power up. INI files contain a list of EXDOS commands and are produced on the word processor. This file must reside in the "root" directory.
...
A **START** file is usually a file containing a BASIC or machine code program which can be auto RUN by pressing **Fkey 1** (START). This works in a similar way to the tape filing system and no other program should be resident in memory or this will RUN instead.
